...
data sums; 
	set generate;
	x = x**x / PDF('UNIFORM', x, 0,1); *f(x)=x ^ x;
run; 
...
data cumulative;
	set cumulative_sums;
	mean_cum = sum_cum / M;
	error_cum = abs(0.783431 - mean_cum);
...
data sums; 
	set generate;
	h = 0.5;
	x_ = (x**x) / ((x <= &h)*2*x/&h + (x>&h) * 2*(1-x)/(1-&h)); *f(x)=x ^ x;
run; 
...
data sums; 
	set generate;
	x = x**x / pdf('beta',x,&alpha,&beta); *f(x)=x ^ x;
run; 
...
data sums; 
	set generate;
	x = x**x / (2*(1 - &alpha)*x + &alpha); *f(x)=x**x;
run; 
